...
const Footer = () => {
  ...
  const copyToClipboard = (text: string | undefined, successMessage = 'Kopyalandı!') => {
    if (!text) {
      toast.error('Kopyalanacak veri bulunamadı');
      return false;
    }
            ...
            {loading ? (
              ...
              <ul className="space-y-2">
                {categories.map((category) => (
                  <li key={category.id}>
                    <Link
                      to={`/menu?category=${category.name || category.id}`}
              ...
              <p className="text-sm text-gray-400">Henüz kategori eklenmemiş.</p>
            )}
          </div>*/}
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
    <div className="bg-gray-800 rounded-lg max-w-md w-full p-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-bold text-white">WiFi Bilgileri</h3>
        <button 
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-1">Ağ Adı (SSID)</label>
          <div className="flex">
            <input
              type="text"
            ...
            />
            <button
              onClick={() => copyToClipboard(settings.wifi_name)}
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col items-center md:flex-row md:justify-between">
            <p className="text-sm text-gray-500">
              &copy; {currentYear} {settings.restaurant_name || 'Restoran'}. Tüm hakları saklıdır. || <a href='https://www.marchyazilim.com.tr/' target='_blank'>march yazılım</a>
            </p>
            <div className="mt-4 md:mt-0">
              <ul className="flex space-x-4 text-sm text-gray-500">
                ...
                <li><Link to="/feedback" className="hover:text-primary">İletişim</Link></li>
              </ul>
            </div>
          </div>
        </div>
