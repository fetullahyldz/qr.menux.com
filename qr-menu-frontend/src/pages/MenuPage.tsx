      ...
      {loading ? (
        ...
        <>
          {products.length === 0 ? (
            ...
            <div className="flex flex-col gap-4 md:grid md:grid-cols-3 md:gap-6 lg:grid-cols-4">
              {products.map((product) => (
                <div
                  key={product.id}
                  className="flex bg-white rounded-xl shadow-md overflow-hidden transition-transform hover:shadow-lg md:block"
                  onClick={() => loadProductDetails(product.id)}
                >
                  {/* Mobil için yatay düzen */}
                  <div className="flex-shrink-0 w-32 h-32 md:w-full md:h-40 relative">
                    <img
                      src={product.image_url || `https://via.placeholder.com/300?text=${encodeURIComponent(product.name)}`}
                      alt={product.name}
                      className="w-full h-full object-cover md:rounded-t-xl"
                    />
                  ...
                  <div className="flex-1 p-4 flex flex-col justify-between md:block">
                    <div>
