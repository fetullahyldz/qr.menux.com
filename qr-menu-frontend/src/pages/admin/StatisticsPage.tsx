...
const SalesReport = ({ data }: { data: ISalesData[] }) => (
  <div className="mt-6 overflow-hidden rounded-lg bg-white shadow-md">
    <div className="p-4 sm:p-6">
      <h2 className="mb-4 text-base sm:text-lg font-semibold text-gray-800">Satış Raporu</h2>
    </div>
    <div className="space-y-4 sm:overflow-x-auto sm:space-y-0">
      {/* Mobil için kart düzeni */}
      <div className="sm:hidden flex flex-col gap-4 px-4 pb-4">
        {data.length > 0 ? (
          data.map((item, index) => (
            <div key={index} className="bg-gray-50 rounded-lg p-4">
              <div className="flex flex-col gap-2">
                <div className="text-sm font-medium text-gray-800">Tarih: {item.date}</div>
                <div className="text-sm text-gray-600">Sipariş Sayısı: {ensureNumeric(item.orders)}</div>
                <div className="text-sm text-gray-600">Ciro: {ensureNumeric(item.revenue).toFixed(2)} ₺</div>
              </div>
            </div>
          ))
        ) : (
          <div className="text-center text-sm text-gray-500 py-4">Veri bulunamadı</div>
        )}
        {data.length > 0 && (
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex flex-col gap-2">
              <div className="text-sm font-medium text-gray-800">Toplam</div>
              <div className="text-sm text-gray-600">
                Sipariş Sayısı: {data.reduce((sum, item) => sum + ensureNumeric(item.orders), 0)}
              </div>
              <div className="text-sm text-gray-600">
                Ciro: {data.reduce((sum, item) => sum + ensureNumeric(item.revenue), 0).toFixed(2)} ₺
              </div>
            </div>
          </div>
        )}
      ...
      <table className="hidden sm:table w-full table-auto border-collapse">
        <thead className="bg-gray-50">
          <tr>
            <th className="border-b border-gray-200 px-4 sm:px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
              Tarih
            </th>
            <th className="border-b border-gray-200 px-4 sm:px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
              Sipariş Sayısı
          ...
          {data.length > 0 ? (
            data.map((item, index) => (
              <tr key={index} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                <td className="whitespace-nowrap px-4 sm:px-6 py-4 text-sm text-gray-800">{item.date}</td>
                <td className="whitespace-nowrap px-4 sm:px-6 py-4 text-sm text-gray-800">{ensureNumeric(item.orders)}</td>
                <td className="whitespace-nowrap px-4 sm:px-6 py-4 text-sm text-gray-800">
                  {ensureNumeric(item.revenue).toFixed(2)} ₺
                </td>
              </tr>
            ))
          ) : (
        ...
        {data.length > 0 && (
          <tfoot className="bg-gray-50">
            <tr>
              <td className="whitespace-nowrap px-4 sm:px-6 py-4 text-sm font-medium text-gray-800">Toplam</td>
              <td className="whitespace-nowrap px-4 sm:px-6 py-4 text-sm font-medium text-gray-800">
...
const TopProducts = ({ products }: { products: ITopProduct[] }) => (
  <div className="mt-6 overflow-hidden rounded-lg bg-white shadow-md">
    <div className="p-4 sm:p-6">
      <h2 className="mb-4 text-base sm:text-lg font-semibold text-gray-800">En Çok Sipariş Edilen Ürünler</h2>
    </div>
    <div className="space-y-4 sm:overflow-x-auto sm:space-y-0">
      {/* Mobil için kart düzeni */}
      <div className="sm:hidden flex flex-col gap-4 px-4 pb-4">
        {products.length > 0 ? (
          products.map((product) => (
            <div key={product.id} className="bg-gray-50 rounded-lg p-4">
              <div className="flex flex-col gap-2">
                <div className="text-sm font-medium text-gray-800">Ürün: {product.name}</div>
                <div className="text-sm text-gray-600">Sipariş Sayısı: {ensureNumeric(product.order_count)}</div>
                <div className="text-sm text-gray-600">
                  Toplam Ciro: {ensureNumeric(product.total_revenue).toFixed(2)} ₺
                </div>
              </div>
            </div>
          ))
          ...
          {products.length > 0 ? (
            products.map((product) => (
              <tr key={product.id} className="hover:bg-gray-50">
                <td className="whitespace-nowrap px-4 sm:px-6 py-4 text-sm text-gray-800">{product.name}</td>
                <td className="whitespace-nowrap px-4 sm:px-6 py-4 text-sm text-gray-800">
                  {ensureNumeric(product.order_count)}
                </td>
                <td className="whitespace-nowrap px-4 sm:px-6 py-4 text-sm text-gray-800">
                  {ensureNumeric(product.total_revenue).toFixed(2)} ₺
                </td>
              </tr>
            ))
          ) : (
...
const FeedbackSummary = ({ feedback }: { feedback: IFeedbackStatistics }) => (
  <div className="mt-6 overflow-hidden rounded-lg bg-white shadow-md">
    <div className="p-4 sm:p-6">
      <h2 className="mb-4 text-base sm:text-lg font-semibold text-gray-800">Müşteri Geribildirimleri</h2>
      <div className="mb-4 flex flex-col sm:flex-row items-start sm:items-center gap-4">
        <div className="text-3xl sm:text-4xl font-bold text-primary">{ensureNumeric(feedback.averageRating).toFixed(1)}</div>
        <div>
          <div className="flex">
            {[1, 2, 3, 4, 5].map((star) => (
              <svg
    ...
    <div className="px-4 sm:px-6 pb-4 sm:pb-6">
      ...
      {feedback.ratings.map((item) => (
        <div key={ensureNumeric(item.rating)} className="mb-2 flex items-center gap-2">
          <div className="w-8 sm:w-10 text-sm">{ensureNumeric(item.rating)} ★</div>
          ...
          <div className="w-8 sm:w-10 text-right text-sm text-gray-600">
...
const SummaryStats = ({ totalOrders, totalRevenue, averageOrder }: { totalOrders: number; totalRevenue: number; averageOrder: number }) => (
  <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
    <div className="rounded-lg bg-white p-4 sm:p-6 shadow-md">
      ...
      <p className="text-2xl sm:text-3xl font-bold text-gray-800">{ensureNumeric(totalOrders)}</p>
    </div>
    <div className="rounded-lg bg-white p-4 sm:p-6 shadow-md">
      <h2 className="mb-1 text-sm font-medium text-gray-500">Toplam Ciro</h2>
      <p className="text-2xl sm:text-3xl font-bold text-gray-800">{ensureNumeric(totalRevenue).toFixed(2)} ₺</p>
    </div>
    <div className="rounded-lg bg-white p-4 sm:p-6 shadow-md">
      <h2 className="mb-1 text-sm font-medium text-gray-500">Ortalama Sepet</h2>
      <p className="text-2xl sm:text-3xl font-bold text-gray-800">{ensureNumeric(averageOrder).toFixed(2)} ₺</p>
    </div>
  </div>
);
...
const WeeklySalesReport = ({ data }: { data: IWeeklySalesData[] }) => (
  <div className="mt-6 overflow-hidden rounded-lg bg-white shadow-md">
    <div className="p-4 sm:p-6">
      <h2 className="mb-4 text-base sm:text-lg font-semibold text-gray-800">Haftalık Satış Raporu</h2>
    </div>
    <div className="space-y-4 sm:overflow-x-auto sm:space-y-0">
      {/* Mobil için kart düzeni */}
      <div className="sm:hidden flex flex-col gap-4 px-4 pb-4">
        {data.length > 0 ? (
          data.map((item, index) => (
            <div key={index} className="bg-gray-50 rounded-lg p-4">
              <div className="flex flex-col gap-2">
                <div className="text-sm font-medium text-gray-800">Tarih: {item.day_name}</div>
                <div className="text-sm text-gray-600">Sipariş Sayısı: {ensureNumeric(item.orders)}</div>
                <div className="text-sm text-gray-600">Ciro: {ensureNumeric(item.revenue).toFixed(2)} ₺</div>
              </div>
            </div>
      ...
      <table className="hidden sm:table w-full table-auto border-collapse">
        ...
        <tbody className="divide-y divide-gray-200 bg-white">
          {data.length > 0 ? (
            data.map((item, index) => (
              <tr key={index} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                <td className="whitespace-nowrap px-4 sm:px-6 py-4 text-sm text-gray-800">{item.day_name}</td>
                <td className="whitespace-nowrap px-4 sm:px-6 py-4 text-sm text-gray-800">{ensureNumeric(item.orders)}</td>
                <td className="whitespace-nowrap px-4 sm:px-6 py-4 text-sm text-gray-800">
                  {ensureNumeric(item.revenue).toFixed(2)} ₺
                </td>
  ...
  return (
    <div className="container mx-auto py-6 px-4 sm:px-6 lg:px-8">
      <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-800">İstatistikler ve Raporlar</h1>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
          ...
          <button
            className="w-full sm:w-auto rounded-md bg-primary px-3 py-2 text-sm text-white disabled:bg-primary-light"
      ...
      {loading ? (
        ...
        <>
          ...
          {period === 'week' && weeklyData.length > 0 ? <WeeklySalesReport data={weeklyData} /> : <SalesReport data={salesData} />}
