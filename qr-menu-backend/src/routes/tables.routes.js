...
router.post('/', authenticateToken, authorizeRoles('admin', 'manager'), async (req, res) => {
  try {
    ...
    const existingTable = await db.getOne('SELECT id FROM restaurant_tables WHERE table_number = ?', [table_number]);
    if (existingTable) {
      return res.status(400).json({
        success: false,
    ...
    // Masa için QR kodu oluştur
    const qrCodeUrl = await generateQrCodeForTable(tableId, table_number);
...
router.put('/:id', authenticateToken, authorizeRoles('admin', 'manager'), async (req, res) => {
  try {
    ...
    if (regenerate_qr === true || regenerate_qr === 'true') {
      const newQrCodeUrl = await generateQrCodeForTable(tableId, table_number || table.table_number);
...
// Masanın durumunu güncelle (boş, dolu, rezerve)
