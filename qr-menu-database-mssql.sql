...
BEGIN
    CREATE TABLE [dbo].[order_items] (
        ...
        CONSTRAINT CHK_order_items_status CHECK ([status] IN ('preparing', 'ready'))
    );
END
GO
...
BEGIN
    ...
    VALUES (@ana_yemekler_id, 'Izgara Köfte', 'Özel baharatlarla hazırlanmış el yapımı köftemiz', 120.00, 1, 1, 0);

    DECLARE @kofte_id INT;
    SET @kofte_id = SCOPE_IDENTITY();

    INSERT INTO products (category_id, name, description, price, is_active, sort_order)
    VALUES (@ana_yemekler_id, 'Tavuk Şiş', 'Marine edilmiş tavuk parçaları', 100.00, 1, 2, 0);
    ...
    -- İçecekler ürünleri
    INSERT INTO products (category_id, name, description, price, is_active, sort_order)
    VALUES (@icecekler_id, 'Ayran', 'Ev yapımı taze ayran', 15.00, 1, 1, 0);

    INSERT INTO products (category_id, name, description, price, is_active, sort_order)
    VALUES (@icecekler_id, 'Kola', '330ml kutu', 20.00, 1, 2, 0);

    -- Tatlılar ürünleri
    INSERT INTO products (category_id, name, description, price, is_active, sort_order)
    VALUES (@tatlilar_id, 'Kazandibi', 'Geleneksel Türk tatlısı kazandibi', 45.00, 1, 1, 0);

    INSERT INTO products (category_id, name, description, price, is_active, sort_order)
    VALUES (@tatlilar_id, 'Sütlaç', 'Fırında pişirilmiş geleneksel sütlaç', 40.00, 1, 2, 0);
